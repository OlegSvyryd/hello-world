...
<div id="wrapper">
    ...
    <form:form id = "add_store_form" method="POST" action="/upd_company_goods_success" commandName="company_goods">
        <form:hidden path="id"/>
        <form:hidden path="company.id"/>
        <form:hidden path="goodsType.id"/>

        <table id = "add_store_table">
            <caption><h3>Редагування даних про товар</h3></caption>
            <tr>
                <th class="too-width"><form:label path="name">Назва товару(попередня назва - ${company_goods.name})</form:label></th>
            </tr>
            <tr>
                <td><form:input path="name" cssClass="error"/><form:errors path="name" /></td>
            </tr>
            <tr>
                <th><form:label path="price">Ціна товару(попередня ціна - ${company_goods.price})</form:label></th>
            </tr>
            <tr>
                <td><form:input path="price" cssClass="error"/><form:errors path="price" /></td>
            </tr>
            <tr>
                <th><form:label path="amount">Кількість на складі(зараз на складі - ${company_goods.amount})</form:label></th>
            </tr>
            <tr>
                <td><form:input path="amount" cssClass="error"/><form:errors path="amount" /></td>
            </tr>
            <tr>
                <th class="too-width"><form:label path="description">Дані про товар(попередні дані - ${company_goods.description})</form:label></th>
            </tr>
            <tr>
                <td><form:textarea path="description" cssClass="error"/><form:errors path="description" /></td>
            </tr>
            <tr>
                <th><form:label path="dimension">Одиниці виміру товару(попередня одиниця - ${company_goods.dimension})</form:label></th>
            </tr>
            <tr>
